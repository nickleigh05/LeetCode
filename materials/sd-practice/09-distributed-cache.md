...
**Inside one node: the LRU you already built.** Each node is a **hash map + doubly linked list** — O(1) get, O(1) set, O(1) evict-least-recent, exactly the [LRU cache](../data-structures/lru-cache.md) from the [linked-list lesson](../learning/07-linked-list.md). This is the best DSA-to-systems bridge on the ladder — take the ten seconds to draw it. Memory is managed with **slab allocation** (namecheck: fixed-size chunks per size class, so long-running processes don't fragment).
